...
絶対条件：
- お客様の選択項目（purpose / vibe / avoid_colors / nail_duration / age）を最優先で踏襲
- 奇抜すぎない：新しさは10〜20%程度（少しだけ新しい自分を発見できる）
- 日常に馴染む範囲で、洗練された雰囲気
- ベージュ単色に寄りすぎない（ただし落ち着きは保つ）
- ストーン/強ラメ/派手ホイルは基本使わない（使うなら極小で1本だけ）
